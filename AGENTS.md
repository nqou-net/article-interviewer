...
6. **セッション解説者エージェント**: AI エージェントのセッションログを解説する際に使用。GitHub Copilot Tasks などのセッションを初心者向けにわかりやすく説明する記事を作成します。
...
**セッション解説パターン**: AI エージェントのセッションを教材として活用する場合
1. 解説対象となるセッションログ（GitHub Copilot Tasks のPR など）を取得
2. セッション解説者エージェントでセッションの流れを分析
3. 初心者向けの段階的な解説文を作成
4. コンテンツ戦略エージェントで記事構成を整理
5. テクニカルライターエージェントで最終的な記事に仕上げる
6. 用語集や FAQ を充実させて初心者の理解をサポート
